...
Version 2.10 adds `S4.construction_intro`, a separate Construction opening from canonical S3 preparation, changes, representations and gaps, and S4 learning and model-landscape statements. The short learning task remains unchanged. The synthesis is INTERPRETED; individually inspectable statements retain their own statuses. References carry their owning stage so matching S3/S4 evidence IDs remain distinct. A failed introduction falls back to the short learning task. Qualification: `outputs/UX_Construction_Rollout_2026-09-07/RELEASE.md`.
