...
(() => {
  ...
  const state = {audit:null,library:[],runtime:null,lastAudit:null,section:'overview',model:0,metric:null,view:0,filter:'ALL',outcomeSort:'priority',sourceFilter:'ALL',routeIndex:0,representationIndex:0,approachIndex:0,evidenceTopic:0,compareLeft:0,compareRight:1,details:new Map(),counter:0,route:0,drawerFocus:null,drawerStack:[],drawerRequest:0,pending:false,search:'',libraryLayout:'grid',lastProgress:null,homePurposeCache:{},homePurposeLoading:new Set(),homePurposeTask:null,privateEpoch:0,sessionChecking:false,auth:{ready:false,enabled:false,authenticated:false,hostedDemo:false},loginPending:false,loginError:'',loginUsername:'',loginFocus:null};
  ...
  function component(id) {
    ...
    if(!c||!canonicalDisplayFields.has(id))return c;
    return {...c,entries:c.entries.map(e=>typeof e.canonical_text==='string'&&e.canonical_text.trim()?{...e,decision:'KEEP',content:c.component==='LIST'?{items:[e.canonical_text]}:{text:e.canonical_text}}:e)};
  }
  ...
  function publication() {
    ...
    return `<div class="publication-note"><span class="badge ${p.status==='PUBLISHABLE'?'documented':p.status==='NOT_PUBLISHABLE'?'unresolved':'interpreted'}">${esc(pretty(p.status))}</span>${state.audit.editorial_mode==='CANONICAL'?'<span>Canonical copy</span>':''}<button data-detail="${k}">Status and limitations</button></div>`;
  }
  function confidence() {
  ...
  function shape(expanded=false) {
    ...
    const statement=n.statement||'Not available',split=splitShapeStatement(statement);
    return `<article class="panel shape-card ${expanded?'shape-expanded':''} ${split.scalar?'has-count':''}"><header class="shape-header"><h2>${icon('data-shape')}Data Shape</h2><div class="shape-headline"><div class="shape-count">${badge(n,{title:'N',stage:'Q1',canonical_text:statement},!expanded)}${split.scalar?`<strong>${esc(split.scalar)}</strong>`:'<span class="sr-only">N</span>'}</div><p>${esc(split.caption)}</p></div></header><div class="shape-fields">${[['unit','Unit'],['target','Target'],['split','Split'],['representation','Representation']].map(([key,label])=>{
      const f=profile[key]||{};return `<div class="shape-row"><span class="shape-label">${grid}${label}</span>${badge(f,{title:label,stage:'Q1',canonical_text:f.statement},true)}<span class="shape-value">${esc(f.statement||'Not available')}</span></div>`;
    }).join('')}</div></article>`;
  }
  ...
  function referenceInventory(){
    ...
    for(const c of state.audit.components||[]){for(const part of [c,...(c.explanation?[c.explanation]:[])])for(const e of part.entries||[])for(const ref of e.metadata?.evidence||[])add(ref,part.stage,part.label);}
  ...
  function showDetail(d) {
    ...
    if(refs.length)body+=section('Evidence references',refs.map(r=>`<button class="evidence-link" data-evidence="${esc(r.evidence_id)}" data-stage="${esc(d.stage||'')}" data-ref-detail="${detail({title:r.evidence_id,record:r})}"><strong>${esc(r.evidence_id)}</strong><span>${esc(r.artifact||'')} · ${esc(r.location||'')}</span></button>`).join(''));
...
if(event.target.id==='preview-select'){state.view=Number(event.target.value);drawAudit();$('preview-select')?.focus();}});
  ...
  document.addEventListener('submit',async event=>{
    if(event.target.id==='login-form'){
      ...
      const form=event.target,username=form.elements.username.value,password=form.elements.password.value,remember=form.elements.remember.checked;state.loginUsername=username;
