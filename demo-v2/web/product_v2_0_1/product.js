...
(() => {
  ...
  const state = {audit:null,library:[],runtime:null,lastAudit:null,section:'overview',model:0,metric:null,view:0,filter:'ALL',outcomeSort:'priority',sourceFilter:'ALL',routeIndex:0,representationIndex:0,approachIndex:0,evidenceTopic:0,compareLeft:0,compareRight:1,details:new Map(),counter:0,route:0,drawerFocus:null,drawerStack:[],drawerRequest:0,pending:false,search:'',libraryLayout:'list',lastProgress:null,homePurposeCache:{},homePurposeLoading:new Set(),homePurposeTask:null,privateEpoch:0,sessionChecking:false,auth:{ready:false,enabled:false,authenticated:false,hostedDemo:false},loginPending:false,loginRemember:false,loginError:'',loginUsername:'',loginFocus:null};
  ...
  const visualEvidence = sectionName => window.BovedaVisualEvidence?.render(state.audit?.visual_evidence,sectionName) || '';
  window.BovedaVisualEvidence?.bind($('main'));
  ...
  function supervisorySignals(){
    ...
    return `<article class="panel signals-overview"><div class="signals-question"><p class="eyebrow">Supervisory Signals</p><h2>What needs<br> closer review?</h2><p class="muted">${rows.length} recorded outcomes. These describe supervisory checks, not a model-quality score.</p></div><div class="outcome-summary" aria-label="Signal outcomes">${outcomes.map(outcome=>`<button data-filter="${outcome}" aria-pressed="${state.filter===outcome}" class="outcome-card ${cls(outcome)} ${state.filter===outcome?'active':''}"><strong>${rows.filter(r=>r.outcome===outcome).length}</strong><span>${outcome}</span><span class="outcome-dots" aria-hidden="true">${rows.map(r=>`<i class="${r.outcome===outcome?'on':''}"></i>`).join('')}</span></button>`).join('')}</div></article><section class="outcome-review"><header><h2>${state.filter==='ALL'?'All supervisory outcomes':esc(state.filter)}</h2><button data-filter="ALL" class="text-button" aria-pressed="${state.filter==='ALL'}">Show all ${rows.length} outcomes</button></header><div class="signal-table" role="table" aria-label="Supervisory outcomes"><div class="signal-table-head" role="row"><span role="columnheader">Check</span><span role="columnheader" class="outcome-sort-header" aria-sort="${state.outcomeSort==='priority'?'ascending':'descending'}"><button type="button" data-outcome-sort="toggle" class="outcome-sort ${state.outcomeSort==='priority'?'':'reversed'}" aria-label="Outcome: ${state.outcomeSort==='priority'?'Signal first. Sort Not applicable first':'Not applicable first. Sort Signal first'}" title="${state.outcomeSort==='priority'?'Signal → Clear → Insufficient evidence → Not applicable':'Not applicable → Insufficient evidence → Clear → Signal'}">Outcome <img src="/product_v2_0_1/assets/outcome-sort.svg" alt="" aria-hidden="true"></button></span><span role="columnheader">Explanation</span><span role="columnheader" class="signal-inspect-header" aria-label="Inspect"><img class="inspect-chevron" src="/product_v2_0_1/assets/inspect-chevron.svg" width="14" height="14" alt="" aria-hidden="true"></span></div><div class="signal-table-body">${orderedRows.map(r=>`<button class="signal-result-row" role="row" data-detail="${detail({title:r.name||r.signal_id,stage:'S5',record:r,context:'A signal outcome is a supervisory finding, not an epistemic status.'})}" aria-label="Inspect ${esc(r.name||r.signal_id)}: ${esc(r.outcome||'Outcome not available')}"><span role="cell" class="signal-result-check">${grid}<span>${esc(r.name||r.signal_id)}</span></span><span role="cell" class="signal-result-outcome"><span class="badge ${cls(r.outcome)}">${esc(r.outcome||'Not available')}</span></span><span role="cell" class="signal-result-explanation">${esc(r.explanation||r.applicability?.explanation||'Open the retained check to inspect the available detail.')}</span><span role="cell" class="signal-result-inspect"><img class="inspect-chevron" src="/product_v2_0_1/assets/inspect-chevron.svg" width="14" height="14" alt="" aria-hidden="true"></span></button>`).join('')||'<p class="empty-copy">No recorded outcomes in this category.</p>'}</div></div></section>`;
  }
  function signals(){return supervisorySignals();}
  function visuals(){return visualEvidence('visuals') || '<section class="visuals-empty panel panel-body"><p class="eyebrow">Additional material</p><h2>Visuals</h2><p class="muted">No visuals have been added to this audit.</p></section>';}
  function auditPage() {
    const a=state.audit;const tab=['overview','evidence','construction','signals','visuals'].includes(state.section)?state.section:'overview';
    const auditHref=s=>`#audit/${encodeURIComponent(a.id)}/${s}`;
    const auditTab=(s,l,extra='')=>`<a href="${auditHref(s)}" class="audit-tab audit-tab-${s} ${extra} ${tab===s?'active':''}" ${tab===s?'aria-current="page"':''}>${s==='overview'?grid:s==='visuals'?'<img class="icon" src="/product_v2_0_1/assets/visuals.svg" alt="" aria-hidden="true">':icon(s)}${l}</a>`;
    const overflowActive=['signals','visuals'].includes(tab);
    return `<header class="audit-heading ${tab==='visuals'?'visuals-heading':''}"><p class="audit-identifier" title="${esc(a.id)}"><span>${esc(a.id.startsWith('audit_')?'Audit':a.id.replace('_Fresh',' · Fresh snapshot'))}</span><span aria-hidden="true">|</span>${repositoryLink(a)}</p><h1>${esc(a.title||a.id)}</h1><nav class="audit-tabs" aria-label="Audit sections">${auditTab('overview','Overview')}${auditTab('evidence','Evidence')}${auditTab('construction','Construction')}${auditTab('signals','Signals')}${auditTab('visuals','Visuals','audit-tab-extra')}<details class="audit-tab-overflow"><summary class="${overflowActive?'active':''} ${tab==='construction'?'construction-active':''}" aria-label="More audit sections" title="More audit sections"><span aria-hidden="true">•••</span></summary><div class="audit-tab-overflow-menu">${auditTab('construction','Construction')}${auditTab('signals','Signals')}${auditTab('visuals','Visuals','audit-tab-extra')}</div></details></nav></header>${a.publication?.status==='NOT_PUBLISHABLE'?'<section class="not-publishable"><h2>This audit is not publishable</h2><p>The audit did not establish a complete analytical result. Its status and limitations are preserved.</p></section>':''}${a.available===false?'<article class="panel panel-body"><h2>Not available</h2><p class="muted">No usable final output is available for this audit.</p></article>':tab==='evidence'?evidence():tab==='construction'?construction():tab==='signals'?signals():tab==='visuals'?visuals():overview()}`;
  }
  ...
  document.addEventListener('click',async event=>{
    document.querySelectorAll('.audit-tab-overflow[open]').forEach(menu=>{if(!menu.contains(event.target))menu.open=false;});
  ...
  document.addEventListener('keydown',event=>{if(event.key==='Escape')document.querySelectorAll('.audit-tab-overflow[open]').forEach(menu=>{menu.open=false;menu.querySelector('summary')?.focus();});});
