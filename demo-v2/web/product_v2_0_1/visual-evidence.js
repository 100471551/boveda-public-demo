(() => {
  ...
  function render(payload, section) {
    ...
    return `<section class="visual-evidence" aria-labelledby="visual-evidence-title-${esc(section)}" data-visual-evidence-status="${esc(payload.status)}"><header class="visual-evidence-heading"><p class="eyebrow">Additional material</p><h2 id="visual-evidence-title-${esc(section)}">Figures in context</h2></header><div class="visual-evidence-grid">${cards.join('')}</div><dialog class="visual-dialog" aria-labelledby="visual-dialog-title-${esc(section)}"><div class="visual-dialog-frame"><header><div><p class="eyebrow">Visual evidence</p><h2 id="visual-dialog-title-${esc(section)}"></h2></div><button class="icon-button visual-dialog-close" type="button" data-visual-close aria-label="Close enlarged figure"><img src="/product/assets/close.svg" alt=""></button></header><figure><img alt=""><figcaption></figcaption></figure></div></dialog></section>`;
  }
